...
import { Search, ChevronDown, MessageCircle, Mail, HelpCircle, FileText, ShieldQuestion, Zap, User } from 'lucide-react';

interface FaqItem {
  question: string;
  answer: string;
  category: 'GENERAL' | 'ACCOUNT' | 'TRANSACTIONS' | 'SECURITY';
}

const FAQS: FaqItem[] = [
  {
    category: 'GENERAL',
    question: 'Apa itu FinTrack AI?',
    answer: 'FinTrack AI adalah aplikasi manajemen keuangan pribadi yang menggunakan kecerdasan buatan (AI) untuk membantu Anda mencatat, menganalisis, dan merencanakan keuangan dengan lebih cerdas dan efisien.'
  },
  {
    category: 'TRANSACTIONS',
    question: 'Bagaimana cara menggunakan AI Smart Entry?',
    answer: 'Cukup ketik transaksi Anda dalam bahasa sehari-hari di menu "Input AI". Contoh: "Makan siang nasi goreng 25rb tadi". AI kami akan otomatis mendeteksi kategori, jumlah, dan tanggal transaksi untuk Anda.'
  },
  {
    category: 'TRANSACTIONS',
    question: 'Apakah saya bisa mengedit transaksi yang sudah disimpan?',
    answer: 'Tentu saja. Pergi ke menu "Riwayat", cari transaksi yang ingin diubah, lalu klik ikon pensil (Edit) di sebelah kanan baris transaksi tersebut.'
  },
  {
    category: 'ACCOUNT',
    question: 'Bagaimana cara mereset password saya?',
    answer: 'Anda dapat mereset password melalui menu "Profil Saya" > "Ganti Password". Jika Anda lupa password saat login, silakan hubungi administrator sistem.'
  },
  {
    category: 'SECURITY',
    question: 'Apakah data keuangan saya aman?',
    answer: 'Ya, data Anda disimpan secara lokal di perangkat Anda (Local Storage) dan tidak dikirim ke server eksternal kecuali untuk pemrosesan AI (yang anonim). Kami menyarankan Anda melakukan backup data secara berkala melalui menu Pengaturan.'
  },
  {
    category: 'GENERAL',
    question: 'Apakah aplikasi ini gratis?',
    answer: 'Saat ini FinTrack AI dapat digunakan sepenuhnya secara gratis dengan fitur-fitur dasar dan AI integration.'
  }
];
          ...
          <div className="space-y-4">
            {filteredFaqs.length === 0 ? (
              ...
              filteredFaqs.map((faq, index) => (
                <div 
                  key={index} 
                  className={`bg-white dark:bg-slate-900 rounded-2xl border transition-all duration-300 overflow-hidden ${openIndex === index ? 'border-indigo-200 dark:border-indigo-800 shadow-md' : 'border-slate-200 dark:border-slate-800 shadow-sm hover:border-indigo-100'}`}
                >
                  <button 
                  ...
                  >
                    <div className="flex items-center gap-4">
                      <div className={`p-2 rounded-lg ${openIndex === index ? 'bg-indigo-100 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400' : 'bg-slate-100 dark:bg-slate-800 text-slate-500'}`}>
                        {(() => {
                          const Icon = categories.find(c => c.id === faq.category)?.icon || HelpCircle;
                          return <Icon className="w-5 h-5" />;
                        })()}
                      </div>
                      <span className={`font-bold text-sm ${openIndex === index ? 'text-indigo-900 dark:text-white' : 'text-slate-700 dark:text-slate-300'}`}>
                        {faq.question}
                      </span>
                    </div>
                  ...
                  >
                    <div className="p-5 pt-0 pl-[4.5rem] text-sm text-slate-600 dark:text-slate-400 leading-relaxed border-t border-transparent">
                      {faq.answer}
                    </div>
                  </div>
                </div>
